	...
	$.fn.smartZoom = function (method) {
		...
		var publicMethods = {
		  ...
		  * 	   						           }  if this parameter is missing the zoom will target the object center
			...
			destroy: function () {
				...
				if(settings.animateOnDestroy == true)
			...
			/**
			 * return true if image is zoomed, otherwise return false
			 * @returns {boolean}
			 */
			isZoomed: function(){
				var smartData = targetElement.data('smartZoomData');
		...
		function stopAnim(userActionType) {
			...
			if (smartData.transitionObject) { // if css transformation is surpported
				...
				var scaleW = targetRect.width / originalSize.width;
				var scaleH = targetRect.height / originalSize.height;
				scaleW = scaleW < 1 ? 1 : scaleW;
				scaleH = scaleH < 1 ? 1 : scaleH;
				if (smartData.transitionObject.css3dSupported) {
					cssObject[smartData.transitionObject.transform] = 'translate3d(' + targetRect.x + 'px, ' + targetRect.y + 'px, 0) scale3d(' + scaleW + ',' + scaleH + ', 1)';
				} else {
					cssObject[smartData.transitionObject.transform] = 'translateX(' + targetRect.x + 'px) translateY(' + targetRect.y + 'px) scale(' + scaleW + ',' + scaleH + ')';
				}
		...
		function adjustToContainer() {
			...
			var scaleToFit = Math.min(Math.min(containerRect.width/originalSize.width, containerRect.height/originalSize.height), 1).toFixed(4); // scale to use to include the target into containerRect
		...
		function animate(target, globalLeft, globalTop, width, height, duration, callback) {
			...
			if (smartData.transitionObject != null) { // use css transition if supported
				var originalSize = smartData.originalSize;
				var scaleW = width / originalSize.width;
				var scaleH = height / originalSize.height;
				scaleW = scaleW < 1 ? 1 : scaleW;
				scaleH = scaleH < 1 ? 1 : scaleH;
				...
				if (smartData.transitionObject.css3dSupported) // use css 3d translate if supported
					cssObject[smartData.transitionObject.transform] = 'translate3d(' + left + 'px, ' + top + 'px, 0) scale3d(' + scaleW + ',' + scaleH + ', 1)';
				else
					cssObject[smartData.transitionObject.transform] = 'translateX(' + left + 'px) translateY(' + top + 'px) scale(' + scaleW + ',' + scaleH + ')';
